...
async function runAnomalyDetection(serviceId: string, ts: Date) {
  ...
  const forecastMetrics = ["cpu", "memory"] as const
  for (const metric of forecastMetrics) {
      ...
      if (slope > 0.1 && currentVal < 98) {
          ...
          if (minsToFull > 0 && minsToFull < 60) {
              ...
              const recentForecast = await prisma.anomaly.findFirst({
                  where: {
                      serviceId,
              ...
              if (!recentForecast) {
                  await createAnomaly(
                      serviceId, 
                      'resource_forecast', 
                      minsToFull, // store minutes as value
                      slope,      // store slope as baseline
                      0,          // no z-score
                      ts,
                      `${metric.toUpperCase()} projected to hit 100% in ~${Math.round(minsToFull)} mins`
                  );
              }
          }
      }
